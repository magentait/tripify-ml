...
def generate_hotel(template: dict, variant: int, comments_count: int) -> dict:
    """
    Генерирует один отель на базе реального шаблона.

    Args:
        template: словарь из REAL_HOTELS
        variant: номер вариации (для GPS jitter, цены)
        comments_count: сколько отзывов сгенерировать
    """
    hid = gen_hid()
