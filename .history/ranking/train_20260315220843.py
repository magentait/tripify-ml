...
class HotelRankingTrainer:
    ...
    def cross_validate(
        ...
        """Group K-Fold cross-validation. Returns NDCG@k per fold."""
        ...
        for fold, (train_idx, val_idx) in enumerate(
            ...
            fold_trainer = HotelRankingTrainer(params=self.params)
            fold_trainer.train(train_df, val_df)

            # Evaluate
            X_val, y_val, groups_val = fold_trainer.prepare_features(val_df)
            scores = fold_trainer.model.predict(X_val)
        ...
        for key in results:
            vals = results[key]
            logger.info(
                f"Mean {key}: {np.mean(vals):.4f} +/- {np.std(vals):.4f}"
            )
    ...
    def _compute_ndcg_per_query(
        ...
        results: dict[int, list[float]] = {5: [], 10: [], 20: []}
        idx = 0
        for g in groups:
...
def main():
    ...
    df = gen.generate_dataset(
        n_queries=800, hotels_per_query=(15, 60), seed=42
    )
    logger.info(
        f"Generated {len(df)} samples across {df['qid'].nunique()} queries"
    )
