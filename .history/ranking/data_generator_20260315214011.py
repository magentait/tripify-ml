...
class SyntheticHotelGenerator:
    ...
    def _oracle_relevance(
        ...
        This simulates what a real user would prefer.
        """
        score = 0.0

        # Rating is king
        score += features.get("bayesian_rating", 3.0) * 8.0

        # Review volume (log scale)
        score += features.get("log_review_count", 0) * 1.5

        # Sub-rating quality
        score += features.get("rc_mean", 5.0) * 1.2

        # Price-quality: lower price at same quality = better
        # Normalise: penalise expensive for budget travellers
        ...
        if budget == "low":
            ...
            score -= price * 0.01
        else:  # high — slight preference for premium
            score += min(price * 0.005, 3.0)

        # Budget match bonus
        score += features.get("budget_price_match", 0) * 4.0

        # Facilities richness
        score += features.get("total_facilities", 0) * 0.5
        score += features.get("free_facility_ratio", 0) * 3.0

        # Cancellation & refund
        score += features.get("has_cancellation", 0) * 3.0
        score += features.get("has_refund", 0) * 2.0

        # Location proximity
        dist = features.get("dist_to_user_km", 0)
        score -= min(dist * 0.3, 5.0)

        # Nearby beaches for family/couple
        vtype = user_ctx.get("vacation_type", "solo")
        if vtype in ("family", "couple"):
            beach_dist = features.get("nearby_beaches_min_dist_m", 99999)
            if beach_dist < 1000:
                score += 3.0
            elif beach_dist < 3000:
                score += 1.0

        # Business travellers prefer wifi, gym
        if vtype == "business":
            score += features.get("fac_wifi", 0) * 3.0
            score += features.get("fac_gym", 0) * 2.0

        # Hotel class bonus
        score += features.get("hotel_class", 3) * 1.5

        # Payment flexibility
        score += features.get("accepts_user_currency", 0) * 2.0

        # Add noise to simulate real-world variance
        score += np_rng.normal(0, 2.0)

        # Map continuous score to 0-4 relevance
        # Use percentile-like bucketing
        if score < 20:
            return 0
        elif score < 30:
            return 1
