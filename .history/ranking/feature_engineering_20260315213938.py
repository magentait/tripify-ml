...
class HotelFeatureExtractor:
    ...
    def _payment_features(self, h: dict, f: dict) -> None:
        pm = h.get("payment_methods") or {}
        cash = pm.get("cash_info") or {}
        cards = pm.get("cards_info") or {}

        f["accepts_cash"] = float(bool(cash.get("is_cash")))
        f["accepts_card"] = float(bool(cards.get("is_card")))
        f["cash_currencies_count"] = float(len(cash.get("currency") or []))
        f["card_types_count"] = float(len(cards.get("card_types") or []))
        f["payment_flexibility"] = f["cash_currencies_count"] + f["card_types_count"]

        # Does hotel accept user's preferred currency?
        pref = self.user_context.get("preferred_currency")
        if pref:
            currencies = [c.upper() for c in (cash.get("currency") or [])]
            f["accepts_user_currency"] = float(pref.upper() in currencies)
        else:
            f["accepts_user_currency"] = 0.0
