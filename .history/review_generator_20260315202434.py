...
def generate_reviews(hotel_title, city, country, hotel_class, count):
    # type: (str, str, str, int, int) -> list
    """Один запрос к LLM → все отзывы для отеля."""
    ...
    # Если LLM вернул меньше — добиваем заглушками
    while len(reviews) < count:
        reviews.append({
            "author": random.choice(["Alex", "Maria", "John", "Yuki", "Hans"]),
